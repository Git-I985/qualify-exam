...
class App:
    ...
    def calc_point(self, x):
        return eval(self.function.replace('x', f'({str(x)})'))
    ...
    def create_plot(self, ct_x_steps=None, ct_y_steps=None, int_steps=False):
        ...
        if int_steps:
            ...
            ox_step = float('{:.2f}'.format(ox_step))
            oy_step = float('{:.2f}'.format(oy_step))
...
if __name__ == '__main__':
    try:
        app = App('coef3.txt')
        app.create_points(10, 20, step=0.05)
